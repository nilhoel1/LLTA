...
///             cache). The engine conservatively wipes the abstract state.
struct CacheEvent {
  enum Kind { Access, Barrier };
  Kind Kind;
  uint64_t LineId; ///< valid only when Kind == Access

  static CacheEvent access(uint64_t LineId) { return {Access, LineId}; }
  static CacheEvent barrier() { return {Barrier, 0}; }
};
