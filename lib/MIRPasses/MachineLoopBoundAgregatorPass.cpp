...
static std::vector<JSONLoopBound> loadLoopBoundsFromJSON(const std::string &JSONPath) {
  ...
  outs() << "Loaded " << Bounds.size() << " loop bounds from JSON file: " << JSONPath << "\n";
  return Bounds;
}
...
bool MachineLoopBoundAgregatorPass::runOnMachineFunction(MachineFunction &F) {
  ...
  std::vector<JSONLoopBound> JSONBounds = loadLoopBoundsFromJSON(LoopBoundsJSON);
  std::unordered_map<std::string, unsigned> JSONBoundsByLocation;

  // Create a lookup map by location (filename:line) - ignore column as it may vary
          ...
          if (!isa<PHINode>(I)) {
            if (const DebugLoc &DL = I.getDebugLoc()) {
              ...
              if (It != JSONBoundsByLocation.end()) {
                TripCount = It->second;
                outs() << "    - Got trip count from JSON: " << TripCount << " (location: " << Key << ")\n";
              } else {
